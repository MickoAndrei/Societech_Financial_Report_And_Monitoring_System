...
(function () {
  ...
  function statusPill(entry) {
    ...
    if (entry.active === false) {
      return '<span class="announcementStatusPill inactive">Inactive</span>';
    }
    if (announcementsService.isOngoing(entry)) {
      return '<span class="announcementStatusPill ongoing">Ongoing</span>';
    }
    return '<span class="announcementStatusPill ended">Ended</span>';
  }
  ...
  function renderList() {
    ...
    if (!list) return;
    const announcementsService = window.SocietechAnnouncements;
    const items = announcementsService.getAllAnnouncements();
    list.innerHTML = '';
    ...
    items.forEach((entry) => {
      ...
      div.innerHTML =
        ...
        statusPill(entry) +
        `</div>` +
        `<div class="announcementEntryMeta">` +
        `<span>Posted: ${escapeHtml(announcementsService.formatPostedDate(entry.postedAt))}</span>` +
        `<span>•</span>` +
        `<span>By: ${escapeHtml(entry.author)}</span>` +
        `<span>•</span>` +
        `<span>${escapeHtml(announcementsService.formatEndLabel(entry.endsAt))}</span>` +
        `</div>` +
        `<p class="announcementEntryBody">${escapeHtml(entry.body)}</p>` +
        `<div class="announcementEntryActions">` +
        `<button type="button" class="btn btnSecondary btnSm" data-edit="${entry.id}">Edit</button>` +
        `<button type="button" class="btn btnDanger btnSm" data-delete="${entry.id}">Delete</button>` +
        `</div>`;
    ...
    list.querySelectorAll('[data-edit]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const entry = announcementsService.getById(btn.getAttribute('data-edit'));
    ...
    list.querySelectorAll('[data-delete]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        const entry = announcementsService.getById(id);
